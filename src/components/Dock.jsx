    ...
    useGSAP(() => {
        ...
        const animateIcons = (mouseX, mouseY) => {
            ...
            icons.forEach((icon) => {
                ...
                const center = left - dockLeft + width / 2;

                const distance = Math.abs(mouseX - center);
                const intensity = Math.exp(-(distance ** 2.5) / 25000);

                const isHovered = icon === hovered || icon.contains(hovered);

                gsap.to(icon, {
                    scale: 1 + 0.25 * intensity,
                    y: isHovered ? -15 : 0,    // ⬅ ONLY hovered icon moves upward
                    duration: 0.2,
                    ease: "power2.out",
                });
            });
        ...
        dock.addEventListener("mousemove", handleMouseMove);
        dock.addEventListener("mouseleave", resetIcons); // <-- FIXED
